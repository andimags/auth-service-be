...
describe('PUT /api/users/:user_id', () => {
    ...
    beforeAll(async () => {
        ...
        userWithLowLevelRoleToken = await generateToken(
            userWithLowLevelRole.email,
            defaultPassword
        );
        ...
        userAdminPermission = await Permission.findOne({
            where: { ref_name: 'admin:user', scope: 'global' }
        });
        if (!userAdminPermission)
            throw new AppError(`Permission with ref_name:'admin_user' and scope:'global' is null`);
